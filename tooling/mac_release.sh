...
echo -e "${GREEN}Compiling the OpenMP project '$project_name' into the release binary for the current macOS machine, with aggressive optimizations...${NC}"
...
clang++ \
  ...
  -lomp \
  -Wall -Wextra \
  "$src_dir"/main.cpp -o "$build_dir"/main-release

if [[ $? -eq 0 ]]; then
  echo -e "${GREEN}Compilation was successful.${NC}"
  chmod +x "$build_dir"/main-release 2> /dev/null
else
